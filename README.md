...
> **Links zum Teilen:** Der normale `github.com/piculezzas/Wissensnetz`-Repo-Link zeigt nur Quelltext an, das Tool läuft dort **nicht**. Es läuft nur über GitHub Pages, und zwar in zwei Varianten derselben Datei:
>
> | Wer | Link | Kann |
> |---|---|---|
> | **Betreuerin(nen)** (nur ansehen) | **`https://piculezzas.github.io/Wissensnetz/`** | Nur Ansehen/Anklicken — Highlighting, Verbindungs-Fokus, Literaturliste inkl. Mehrfachauswahl und Vorkommen-Toggle, Pfeil-Filter, Suche, Zoom, Deep-Links. Kein Bearbeiten-Button, kein Boards-Button, Boxen lassen sich nicht verschieben. *(Keine echte Zugriffssperre, nur eine UI-Vereinfachung — für den Zweck aber ausreichend.)* Das ist der **Standard** — auch ein versehentlich verkürzter Link landet nie im Bearbeitungsmodus. |
> | **Leonie** (Weiterarbeit) | `https://piculezzas.github.io/Wissensnetz/?edit=1` | Ansehen **und** Bearbeiten (Boxen verschieben, Text ändern, Boards/JSON-Import-Export) |
>
> Wer immer einen der beiden Links öffnet, sieht **exakt den zuletzt gepushten/eingebackenen Stand** (aktuell `_2_08`) — nie mehr und nie weniger; beide Links liefern dieselbe Datei aus, nur mit unterschiedlich freigeschalteter Bedienung. Eigene Bearbeitungen im Browser (auch von anderen Personen) landen nur im jeweils eigenen Browser-Speicher und werden dadurch nicht automatisch geteilt; siehe die Warnung in Abschnitt 1 und die Erklärung in Abschnitt 8 — dort auch der wichtige Hinweis, warum der Link **ohne** `?edit=1` im **eigenen** Browser evtl. doch eigene, nicht gepushte Änderungen zeigen kann.
...
- **`?edit=1` ist keine separate Kopie, sondern dieselbe Datei mit umgeschalteter Bedienung; ohne diesen Parameter ist die Seite standardmässig eine reine Ansichtsversion.** Ausnahme: lokal geöffnete Dateien (Doppelklick auf `index.html`, `file://`-Adresse) sind davon nicht betroffen und bleiben ohne Parameter voll editierbar — nur online geteilte Links (`https://…`) starten sicherheitshalber im Ansichtsmodus. Für andere Geräte/Browser (z. B. die Betreuerin) heisst das: Sie sehen dort immer genau den zuletzt gepushten Stand — kleinere Anpassungen von Leonie erscheinen dort erst nach dem üblichen Export → Sync → Push. **Achtung im eigenen Browser:** Der Browser-Speicher gehört zur *Web-Adresse als Ganzes* (Domain `piculezzas.github.io`), nicht zur genauen URL mit/ohne `?edit=1` — beide Links teilen sich also denselben Speicher, wenn man sie im selben Browser öffnet. Wer über den `?edit=1`-Link etwas bearbeitet und danach im selben Browser den einfachen Link aufruft, sieht dort ggf. den eigenen, noch nicht gepushten Arbeitsstand statt des sauberen GitHub-Stands. Um wirklich zu sehen, was die Betreuerin sehen würde, entweder ein privates/Inkognito-Fenster nutzen oder einen anderen Browser.
