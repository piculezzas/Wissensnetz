...
- **`?view=1` ist keine separate Kopie, sondern dieselbe Datei mit umgeschalteter Bedienung.** Für andere Geräte/Browser (z. B. die Betreuerin) heisst das: Sie sehen dort immer genau den zuletzt gepushten Stand — kleinere Anpassungen von Leonie erscheinen dort erst nach dem üblichen Export → Sync → Push. **Achtung im eigenen Browser:** Der Browser-Speicher gehört zur *Web-Adresse als Ganzes* (Domain `piculezzas.github.io`), nicht zur genauen URL mit/ohne `?view=1` — beide Links teilen sich also denselben Speicher, wenn man sie im selben Browser öffnet. Wer im normalen Link etwas bearbeitet und danach im selben Browser `…/?view=1` aufruft, sieht dort ggf. den eigenen, noch nicht gepushten Arbeitsstand statt des sauberen GitHub-Stands. Um wirklich zu sehen, was die Betreuerin sehen würde, entweder ein privates/Inkognito-Fenster nutzen oder einen anderen Browser.
